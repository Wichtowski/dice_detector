...
from dice_detector.foundry import FoundryClient, FoundryWebSocketServer, MessageFormatter
...
from dice_detector.roll_engine import PresetManager, RollCalculator
from dice_detector.ui.widgets import (
    DiceDisplayWidget,
    ModifierPanel,
    PresetSelector,
    RollFormulaInput,
...
class MainWindow(ctk.CTk):
    ...
    def __init__(
        ...
        self.settings = CalibrationSettings(camera_index=initial_camera_index)
        self.foundry_config = FoundryConfig()

        self.camera = CameraCapture(self.settings)
        self.vision = VisionPipeline(self.settings, model_path=model_path)
        ...
        self.foundry_ws_server = FoundryWebSocketServer(
            host="127.0.0.1",
            port=self.foundry_config.websocket_port,
        )
    ...
    def _connect_foundry(self):
        ...
        if self.foundry_ws_server.start():
            self.connection_label.configure(
                text=f"● Server Running ({self.foundry_config.websocket_url})",
                text_color="#66ff66",
            )
        else:
