"""Dice value recognition using OCR and image processing."""
...
from dice_detector.models import BoundingBox, DetectedDie, DiceType


class DiceRecognizer:
    """Recognizes dice values from cropped images."""
    ...
    def _count_pips(self, image: np.ndarray) -> tuple[int | None, float]:
        ...
        if 1 <= pip_count <= 6:
            confidence = 0.7 + (0.05 * min(pip_count, 3))
    ...
    def _recognize_numeric(
        ...
        if value is not None:
            ...
            if dice_type == DiceType.D10 and value == 0:
                value = 10
